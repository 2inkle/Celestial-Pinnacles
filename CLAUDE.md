...
몇 번 있었다"고 신고 → 원인 조사 → 그 결과를 정식 게임 디자인으로 채택함.

**원인(조사 결과, 파일:라인 근거)**: 고블린 마차(`goblin_cart`)는 "직접
처치 불가" 컨셉 — `patterns`에 `hp<=50 → DIALOGUE_DEFEAT→REWARD_GRANT→
SELF_DETONATION`이 체인(`chains:true`)으로 걸려 있어서, 정상 공략이면 항상
이 스크립트로 퇴각+보물상자로 마무리된다(`src/engine.js`의 `executeAction()`).
그런데 이 체인은 **마차 자신의 턴에만** 평가되는 반면, 사망 판정(`isAlive`
= `currentHp > 0`, `src/character.js`)은 아군이 때린 그 즉시
`checkForDeaths()`(`src/engine.js`)로 확정된다 — 그래서 아군의 한 타격(또는
한 턴 다단히트 합산)이 **그 순간 마차의 잔여 HP를 한 번에 0 이하로 만들면**,
마차는 다음 자기 턴을 못 받아 체인 자체가 발동할 기회 없이 그냥 죽는다.
조건은 고정된 데미지 숫자가 아니라 "타격 시점의 잔여 HP 대비 상대적" —
HP 몇 %에 있든 그 순간 잔여 HP 이상의 화력이 한 방에 들어가면 발생.

부수 확인: 직접 처치되면 `grantKillReward()`가 마차 자신의 `expReward:15`만
지급하고(골드·드랍 없음 — 전부 보물상자 쪽에 있었음) 원래는 오히려 손해인
결과였음.

**디자인 결정(사용자)**: 이 "우연히 죽일 수 있다"는 현상을 버그로 고치는
대신 **정식 이스터에그로 승격**시킴 — AFTERMATH("???"전) 입장권 "불길한
마력 파편"을 보물상자의 30% 확률 드랍에서 빼고, **마차 본체를 직접
처치했을 때만 100% 확정 드랍**하도록 옮김(`0031_cart_direct_kill_easter_egg.sql`).
정상 공략(퇴각을 지켜보고 보물상자만 부숨)으로는 이제 이 아이템을 절대 못
얻는다 — "마차는 원래 처치 불가"라는 설정을 곧이곧대로 받아들인 플레이어는
평생 못 얻고, "정말 안 죽는지" 화력으로 직접 시험해본 플레이어만 우연히
얻게 되는 구조. 앞으로 레벨 상한 해방에 "???"전 승리가 조건으로 걸릴
예정이라([레벨 상한 30 관련 기존 설계 참고]), 그 시작점 자체("마차를
정말로 죽일 수 있다"는 사실)가 게임 안에 어디에도 안내되지 않는 하나의
비밀로 남는다.

`web/battle-themes.js`의 "불길한 마력 파편은 보물상자 30% 드랍"이라는 낡은
주석도 이번에 같이 정정함. `goblin_cart`의 퇴각/보물상자 메커니즘 자체
(`RETREAT`/`REWARD_GRANT` 액션, 체인 구조)는 전혀 안 건드림 — 오직 "불길한
마력 파편"이 어디서 나오는지만 이전함.

⚠ 이 세션엔 Node.js가 없어 실제 확률/드랍 실행 검증은 못 함 — SQL 문법·
괄호 균형·jsonb 경로만 코드 리뷰로 확인. `game_content.monsterRoster`에
실제로 반영됐는지, 마차를 직접 처치했을 때 파편이 정말 100%로 나오는지는
다음 세션(주 워크스테이션)에서 확인 필요.

## [P0 해결] 파견 전리품 정산 — 킬 카운트 압축 후 독립 재판정으로 재설계 (2026-08-31)

2026-08-25에 발견하고 계속 미뤄뒀던 "[P0] 파견 전리품 정산" 구조 결함(아래
해당 섹션 참고)을 실제로 고쳤다. `web/dispatch.html` 직접 수정(마이그레이션
아님 — 이 파일은 정적 코드가 런타임 소스).

**문제 요약(재정리)**: 예전 방식은 파견 전체에서 쌓인 드랍 "수량"을 그냥
`LOOT_DIVISOR(100)`로 나눴다. 그런데 파견 안에서 같은 몬스터가 몇 번이든
잡힐 수 있다는 전제가 없어서, raw 수량이 100 미만인 아이템(=희귀 아이템 전부)은
몬스터를 몇 번을 잡든 최종 결과가 항상 "0개 또는 1개"로 상한이 걸렸다.
반대로 몬스터가 파견 안에서 300번 넘게 잡히면(고블린 왕 실측 340회),
15% 드랍인 "왕의 대검"도 raw≈51이 되어 **파견 한 번에 획득 확률 ~51%**까지
치솟았다 — "51개어치 파밍한 성과"와 "1개어치 파밍한 성과"가 똑같이 동전
던지기 하나로 뭉개지는 역전 구조였음(자세한 경위는 아래 원본 섹션).

**새 방식**: "몬스터를 죽인 킬 카운트" 자체를 먼저 `LOOT_DIVISOR`로 압축
(확률적 반올림)한 뒤, **압축된 횟수만큼만** `engine.js`의 `grantKillReward`와
동일한 개별 확률 판정으로 드랍을 다시 굴린다(예: 340킬 → 압축 3~4회 →
그 3~4회에 대해서만 각 드랍 아이템을 `Math.random() <= chance`로 독립 재판정).
- 몬스터가 파견에서 아무리 자주 잡혀도, 아이템 하나하나는 **"직접 도전과
  완전히 같은 확률"**을 유지 — 자주 마주친다고 희귀도가 무력화되지 않음.
- 기댓값은 예전과 정확히 동일(선형성 — 압축 시행 횟수의 기댓값이
  `raw킬수/LOOT_DIVISOR`이므로).
- `r.lootGained`(엔진이 전투 안에서 자체적으로 굴린 드랍)는 더 이상 안 씀 —
  대신 dispatch가 `killCounts`(몬스터별 실제 격파 횟수)만 집계하고, 드랍
  판정 자체를 dispatch 쪽에서 직접 재계산함. `allyWin`이면 그 판의
  `enemyKeys` 전원이 격파된 것이 보장되므로(전멸승 조건), 킬 카운트 집계는
  기존 승리 판정 로직에 그대로 얹힘 — 엔진 변경 없음.
- 결과 화면 표시: "현지 XX → ×N"(예전, raw 수량 기준)을 "압축 시행 N회 →
  ×N"(새 방식, 압축된 시행 횟수 기준)으로 갱신. `attempts`는 이 아이템이
  몇 번의 압축 시행 중 나왔는지를 보여주는 참고용 숫자(여러 몬스터가 같은
  이름을 드랍하면 합산됨 — 진짜 판정 근거는 이미 끝났고 표시 전용).

**검증**: 이 세션엔 Node.js가 없어 실제 실행/확률 분포 검증은 못 함 — 코드
리뷰로 (1) 압축 공식이 기존에 이미 제안해뒀던 방향과 정확히 일치하는지,
(2) `engine.js`의 드랍 판정 로직(`Math.random() > drop.chance`이면 실패)과
새 코드(`Math.random() <= drop.chance`이면 성공)가 논리적으로 동치인지,
(3) `finalLoot`이 여전히 `buildWarehouseInsertFromLoot`이 기대하는 필드
(`name`/`category`/`combatReal`/... 전부 `{...drop, quantity, attempts}`로
스프레드돼 보존됨)를 갖는지 확인함. **실제 파견을 돌려 분포가 의도대로
나오는지는 다음 세션(주 워크스테이션, `simulate.js`) 실측 필요.**

**⚠ 여전히 남은 것**: 이 수정은 파견의 "정산 방식"만 고친 것이고, 새 재료를
추가할 때마다 "이 재료가 파견에서 몇 번이나 조우될지"를 매번 사람이 감안해야
하는 부담 자체는 그대로다(예: 심층의 부름을 고블린 왕에게 0.02로 넣을 때도
이 위험을 언급함). 압축 방식이 근본 대응이라 이제 새 아이템을 추가해도
자동으로 안전하지만, 압축 공식이 실제로 의도대로 동작하는지는 아직
`simulate.js` 실측 전이라는 점은 남아있음.

**후속 갱신(2026-09-05, 사용자 확정) — 마이그레이션 진행 후 실측 확인, UI
단순화 + 골드 분할 폐지**: 사용자가 실제로 위 재설계를 굴려보고 "의도한 대로
비슷하게 굴러가는 것"을 확인함. 이어서 두 가지 조정:
1. **결과 화면에서 "압축 시행 N회" 표시 제거** — 전리품 줄에 붙던
   `압축 시행 ${attempts}회 → ×N`을 `×N`(최종 수량)만 남김. 획득 경험치/골드
   카드의 `현지 X ÷ Y` 보조 텍스트, 파견 전 화면의 "보상 정산: 경험치 1/8
   전리품 1/100" 안내 카드도 함께 제거 — 내부적으로 몇 번 나누고 압축하는지는
   더 이상 화면에 노출하지 않고 최종 결과만 보여줌(`attempts` 필드 자체는
   내부 계산엔 여전히 남아있으나 렌더링에서 안 씀).
2. **골드는 더 이상 나누지 않고 전액 지급** — `finalGold = Math.floor(gold /
   EXP_DIVISOR)` → `finalGold = gold`. 사용자 판단: "골드는 분할지급이 필요
   없을것으로 사료된다." 원래 골드가 `EXP_DIVISOR`를 재사용했던 건 CLAUDE.md의
   "API 단계에서 검증/방어가 필요한 지점" 섹션에 이미 "경험치와 같은 변수를
   재사용할 뿐 독립적인 근거가 없다"고 기록돼 있던 부분이라, 이번 결정으로
   그 근거 부족 문제 자체가 해소됨. 경험치(1/8)와 전리품(킬 압축, LOOT_DIVISOR
   100)의 분할 로직 자체는 그대로 유지 — 이번엔 화면 표시와 골드 정산 방식만
   바뀜, 파견 전리품 압축 재판정 로직은 안 건드림.

## 몬스터 레벨 표시 도입 (2026-09-04)

다른 채팅과 나눈 세계관 브레인스토밍(패턴=이 세계의 각본, 레벨 상한 30=투영
해상도, "???"=표기 불가 존재 등)을 이 코드베이스와 대조하는 과정에서 나온
결정 — "몬스터가 얼마나 강한지 단편적으로 보여주는 표시가 있으면 좋겠다,
연출적으로도 써먹을 수 있지 않나"는 사용자 판단으로 몬스터 레벨을 도입함.

**이전 상태**: 몬스터에는 레벨 개념이 아예 없었음(플레이어 캐릭터만 레벨
보유, 몬스터는 `tier:normal/elite/boss`와 순수 스탯만 있었음). `web/monster-roster.html`의
죽은 코드(`LEGACY_MONSTER_SEED`, 런타임 미참조)에 원 설계자가 이미 "고블린
마을 (Lv1~5 구간)"이라 적어둔 걸 근거로, 새로 지어낸 게 아니라 있던 의도를
수치로 확정하는 것에 가까움.

**구현**: `0030_add_monster_levels.sql`이 `game_content.monsterRoster`의 17개
몬스터(0-tier 고블린 8종 + 동굴 9종)에 `jsonb_set`으로 `level` 필드를 추가
(0028/0029와 같은 정밀 병합 패턴, `case elem->>'id' when ... then jsonb_set(...)`).
**`"???"(unknown_entity)`와 `raid_deep_dweller`("심층에서 올라온 것")는 의도적으로
이 분기에서 제외** — 처치 불가/정체불명 존재는 레벨 필드 자체가 안 생기고,
`web/roster-select.html`/`web/monster-roster.html`의 `monster.level ?? "??"`
폴백이 그대로 `Lv.??`를 렌더링함(별도 null 마킹이 아니라 필드 부재로 표현).

레벨 값은 전투 수치에 영향을 주지 않는 순수 표시용 필드라 밸런스 결정이
아님 — 언제든 재조정 가능. 표시 위치는 `web/roster-select.html`의
`renderMonsterPreview()`(2026-08-20에 진형 프리뷰가 제거된 뒤로 전투 진입 전
몬스터 이름이 노출되는 유일한 지점, `battle-view.html:342-348` 주석 참고)와
관리자용 `web/monster-roster.html`의 로스터 카드 목록.

⚠ `raid_deep_dweller`를 "레벨 없음" 쪽에 넣은 건 사용자가 명시적으로 확인한
사항이 아니라 "???"와 같은 처치 불가/방문자 계열이라는 유추 판단임 — 다음
세션에서 사용자 확인 필요.
...
2. ~~소환 아이템 "심층의 부름"의 획득 경로~~ → **해결(아래 "소환 아이템 드랍
   경로" 섹션 참고)**. `0029`(고블린의 왕 dropTable에 추가, 미실행) 남음.
...
## 소환 아이템 "심층의 부름" 드랍 경로 — 고블린의 왕에게 극저확률로 추가 (2026-08-31, 미실행)

**사용자 결정**: "보스 드랍 아이템 중에서도 아주 낮은 확률로 드랍되도록 하겠다."
실제 `dropTable`을 가진 보스는 지금 고블린의 왕(0-tier)뿐이라(동굴 5층 보스는
스탯만 확정, 몬스터 데이터 미작성) 거기 붙임 — 동굴 보스가 생기면 그쪽에도
추가할 수 있음.

`supabase/migrations/0029_king_drops_raid_summon.sql`(신규, **미실행**):
- 확률 **0.02**(2%) — 이 보스의 기존 최저 확률(오래된 바퀴 자국 0.12)보다
  뚜렷하게 낮게 잡음. `category:"consumable"`(0027 헤더의 판단 그대로 —
  keyItem이면 경매 거래에서 빠져 소환서 시장이 안 생김).
- `0023`/`0024`와 같은 정밀 병합 패턴 — `monsterRoster` 배열 전체를 다시 쓰지
  않고 `id="goblin_king"` 원소 하나만 찾아 `dropTable`에 항목을 이어붙임.
  **재실행하면 매번 추가되므로(멱등 아님) 한 번만 실행할 것.**

**⚠ 다시 떠오른 위험 — 파견 loot 구조 결함이 아직 안 고쳐졌다**: "[P0] 파견
전리품 정산" 섹션에서 기록한 대로, 고블린의 왕은 파견 2000턴 예산 안에서
300회 이상 반복 조우된다. 이 확률(0.02)도 파견으로 반복 파밍하면 원본 누적량이
100을 넘어 **사실상 확정 획득**이 될 위험이 있다 — "직접 도전에서만 의미 있는
희귀 아이템"을 새로 추가할 때마다 이 구조적 결함이 매번 되풀이된다. 레이드
소환 아이템이 그 최신 사례. `initBonusDef`처럼 이것도 "다음에 정말 고쳐야 할
선행 과제"로 다시 한번 남겨둠 — 지금은 데이터만 추가하고 구조 개편은 손 안 댐
(범위 밖).

## [P0 선행조건 포함] 타 플레이어 상호작용 — 경매장 + 협동 레이드 (2026-08-31, 설계+스키마 초안, 미실행)

### 이 시스템이 깨는 전제
이 게임은 지금까지 **완전한 1인용**이었다. 조사로 확인한 것:
- `0001_init_schema.sql:292-308` — `profiles`/`characters`/`warehouse_items`/
  `battle_progress`/`shop_purchased`가 전부 `auth.uid() = user_id`로 잠겨 있음.
  `quest_progress`(0020), `battle_logs`(0021)도 동일.
- 남의 데이터를 읽는 경로는 `0021:113-127`의 `get_shared_battle_log` **하나뿐**.
  그 주석이 이유를 명시함: `using(true)` RLS는 PostgREST로 테이블 전체를
  노출시켜서 "id를 아는 사람만"을 RLS로는 표현할 수 없었다.
- Realtime 사용처 0곳. **Edge Function 없음**(`supabase/`에 `migrations/`만) —
  서버 로직은 전부 PL/pgSQL이어야 함.
- 전투는 100% 클라이언트 동기 실행(`battle-adapter.js:572`→`engine.js:152`).
  **서버 재시뮬레이션 불가능.**
- 확장은 `pgcrypto`만 — **pg_cron 없음**.

### ⚠ 최대 리스크 — 골드 위조의 성격이 바뀐다
지금 골드/아이템은 클라이언트가 직접 쓴다. `profiles.update({ gold: 절대값 })`이
9개 페이지에 흩어져 있고(`shop.html:1161,1247`, `dispatch.html:403`,
`battle-view.html:596` 등) RLS는 "본인 행"이라 이를 허용한다.

**1인용에서는 자기기만일 뿐이라 감수할 만했다.** 위조 골드는 고정 카탈로그인
NPC 상점에서만 쓸 수 있어 피해가 본인 진행에 갇혀 있었기 때문. 그런데 경매장이
생기면 **위조 골드가 다른 실제 플레이어의 아이템을 사간다** — 자기완결적이던
위험이 아니게 된다.

**사용자 결정(2026-08-31)**: "경매/레이드 mutation만 RPC로 원자화, 골드 위조는
일단 감수". 이 선을 잡고 진행하되, 위 성격 변화는 기록해둔다. 후속 권장:
append-only `gold_ledger` — 위조를 막지는 못하지만
...
- 새 테이블은 **select 정책만** 만들고 INSERT/UPDATE/DELETE 정책을 **아예 안
  만든다**. RLS가 켜진 테이블은 정책이 없으면 그 동작을 무조건 거부하므로,
  "검사로 막는" 게 아니라 **쓸 수 있는 표면 자체를 없애서** 막는다.
- 읽기는 `using (true)`가 아니라 `auth.role() = 'authenticated'` — 0001의
  `feature_requests` 선례. 로그인 게이트가 걸린 게임이라 익명에게 시장을 열
  이유가 없다.
- **0021과 반대 결론을 내린 이유**: 전투 기록은 "기본 비공개, 링크 소유자만
  예외"라 좁은 RPC가 맞았지만, 경매 매물은 **열거(검색·정렬·필터·페이지네이션)가
  곧 기능**이라 좁은 RPC로 감싸면 PostgREST가 공짜로 주는 걸 다 버리고 필터
  조합마다 RPC를 새로 만들어야 한다.
- 모든 RPC는 행위자를 **함수 안에서 `auth.uid()`로 판별**(파라미터로 받으면
  사칭 가능). **금액도 파라미터로 안 받는다** — `buyout_auction_listing`은 가격
  인자가 없다. 호출자는 "어느 매물"만 지정하고 "얼마"는 서버가 정한다.

### 경매장 — `supabase/migrations/0026_auction_house.sql` (신규, 미실행)
테이블 `auction_listings`/`auction_bids`, RPC 5개(등록/입찰/즉구/취소/만료정산).

핵심 설계 결정:
1. **아이템은 창고에서 빼내서 보관(에스크로)** — 행을 두고 플래그만 세우면
   매물로 올린 채 장착·NPC판매·개조가 가능해 복제가 난다. 등록 시 원본 행을
   삭제하고 `item_snapshot jsonb`에 통째로 담는다.
2. **스냅샷 저장/복원에 컬럼을 절대 나열하지 않는다** — `to_jsonb()` /
   `jsonb_populate_record()`를 쓴다. 0004 헤더가 기록한 "컬럼을 재나열하는
   코드는 나중에 컬럼이 추가될 때 그 필드를 조용히 유실시킨다"는 버그 계열
   방지(실제로 이 테이블은 0004/0022에서 컬럼이 계속 늘어왔다).
3. **입찰 골드는 즉시 차감(에스크로)** + `profiles.gold_locked` 컬럼 신설.
   "낙찰될 때만 청구"가 성립하지 않는 이유: 스케줄러가 없어 정산이 "제3자가
   경매장을 열었을 때" 일어나는데 그 시점에 낙찰자는 거의 확실히 오프라인이다.
   접속 안 한 사람에게서 돈을 걷어야 하고, 돈이 없으면 정산이 실패하거나
   차순위로 내려가는 무한 사다리를 아무 페이지 로드에서나 돌려야 한다.
   에스크로면 **정산 함수가 실패할 수 없다**(이미 걷어둔 돈을 옮기는 장부 작업).
4. **`auction_bids(listing_id) where status='active'` 부분 유니크 인덱스** —
   "매물당 잠긴 입찰은 최대 1건"을 DB가 강제. 환불을 빠뜨린 채 새 입찰을 넣으면
   insert가 실패하므로 골드 누수가 조용히 통과할 수 없다.
5. **`seller_username` 등 이름 비정규화 필수** — `profiles` select 정책이
   "본인 행만"이라 목록 화면에서 판매자 이름을 **조인할 방법이 아예 없다**.
   `battle_logs.saved_by`와 같은 해법. 개명해도 소급 안 되는 건 감수.
6. **가격**: 자유 입력 + 하한(1G)만. 상점의 8단계 고정 라인업은 NPC 전용.
   즉구가 이상으로는 입찰 불가(입찰이 즉구보다 비싸게 낙찰되는 일 방지).
   취소는 입찰 0건일 때만(내렸다 되올리는 시세 조작 방지).
7. **만료는 3층 지연 정산**(pg_cron 부재): ① 경매장 진입 시 스위퍼 호출,
   ② 입찰/즉구가 만료 매물을 건드리면 그 자리에서 정산 후 거절, ③ 목록 쿼리가
   항상 `expires_at > now()`를 함께 검. ③ 덕분에 만료는 의미상 즉시 일어나고
   정산은 장부가 따라오는 일이 된다. 나중에 pg_cron을 쓸 수 있게 되면 스위퍼를
   스케줄에 걸기만 하면 됨(이미 멱등 + `skip locked`라 함수 수정 불필요).

거래 범위: **keyItem 제외 전부**(사용자 확정). 장비는 강화단계/개조 그대로 거래.
(참고: `appraised=false` 미감정품 차단은 검토했으나 **하지 않음** — 감정은
아이템 *이름*으로 `craft-materials.js`를 조회하는 걸 UI에서 가리는 것뿐이고
인스턴스별 숨은 스탯이 아니라서, 매물 공개로 새는 정보가 없음.)

### 협동 레이드 — `supabase/migrations/0027_coop_raid.sql` (신규, 미실행)
테이블 `raid_instances`/`raid_participants`/`raid_runs`, RPC 4개.

모델(사용자 확정): 비동기 누적 데미지 · 양산 가능한 인스턴스제(소환 아이템을
**전투 밖에서** 소모해 개설) · 기믹 성공 시 가산점 · 살아있는 레이드는 전부
공개하되 참여 가능한 것만 필터 · 처치 시 기여도 비례로 전원 분배.

1. **엔진 무변경**: "전투 중 아이템 사용" 원안은 포기했다 — 엔진에 아이템 사용
   액션이 **아예 없다**(`consumable`/`uses_per_battle` 컬럼은 데이터 경로에는
   다 실려 다니지만 `src/` 안에 참조가 **0곳**인 죽은 필드. `ignoreEvade`와 같은
   상태). 대신 전투 밖 RPC로 개설.
2. **레이드 정의는 `game_content.raidTable`**(정적 JS 관례에서 벗어남).
   `battle-themes.js`/`quest-table.js`는 "정적 콘텐츠는 JS로"가 관례지만,
   **기믹 배점이 클라이언트에 있으면 아무나 "999999점 획득"이라 주장할 수 있어**
   서버가 쥐어야 한다. 새 테이블 대신 `game_content` key 목록에 추가 —
   공개읽기/관리자쓰기/updated_at 트리거가 이미 다 붙어 있음.
3. **★ `submit_raid_run`은 데미지 인자를 받지 않는다** — 호출자가 방금 저장한
   `battle_logs`의 `result->'damageDealt'->>'ally'`(`engine.js:309`)에서 읽는다.
   가장 값싸고 효과 큰 방어: "rpc 숫자 하나 바꾸면 끝"을 "앞뒤 맞는 전투 로그를
   통째로 위조해야 하고 그 위조물이 DB에 증거로 남는다"로 바꾼다.
   `raid_runs(battle_log_id)` 유니크 인덱스로 같은 승리 재제출도 차단.
4. **규칙 수치는 개설 시점에 인스턴스로 스냅샷** — raidTable이 바뀌어도 진행 중
   레이드의 규칙이 흔들리지 않게(`battle_logs.battle_name`과 같은 원칙).
5. **기믹은 id만 받고 배점은 서버가** + 유저·레이드당 id 1회만 인정
   (`raid_participants.gimmick_ids`). 무한 점수가 유계 점수가 된다.
   기여도 = `damage + gimmick_points`로 단순 합산 — 배점을 `bossMaxHp` 대비
   비율로 잡으면 "퍼즐 푼 사람이 딜만 넣은 사람보다 낫다"가 그대로 성립.
6. **보상 골드는 최대잉여법(largest remainder)** — 단순 `floor()`만 쓰면 나머지가
   증발해 "총 보상 50만" 표시와 실지급 합계가 어긋난다.
7. **참가 자격 필터링은 RLS가 아니라 클라이언트에서.** 자격이 *보는 사람*의
   상태(입장템 보유/이미 참가/정원)에 달려 있어 RLS로 표현 불가능하고, 애초에
   RLS는 행을 *숨기는* 도구라 "곧 자격이 될 레이드"를 볼 수 없게 만들어
   요구사항("전부 공유하되 필터")과 정면으로 어긋난다.
   `battle-select.html:216-259`의 `REQUIREMENT_TYPES` 레지스트리를 그대로
   재사용하고, 쿨다운을 숨기지 않고 남은 시간을 보여주는 기존 방침도 계승.
   **강제는 RPC가 별도로 다시 한다.**

### 솔직히 못 막는 것
전투가 클라이언트에서 끝나고 서버 재시뮬레이션 수단이 없으므로, 악의적
클라이언트는 여전히 **(1) 앞뒤 맞는 `battle_logs`를 위조해 임의 데미지 주입,
(2) 하지 않은 기믹 주장**이 가능하다. 위 장치들은 전부 "막는" 게 아니라 피해를
유계로 만들고 흔적을 남기는 것:
`max_damage_per_run`(거절이 아니라 **클램프** — 정직한 대박 런도 그만큼은
인정해야 하므로) / `max_attempts_per_user` + 소환템 소모 + `run_cooldown_seconds`
(이건 서버 상태라 진짜로 강제됨) / 기믹 id 1회 인정 / `raid_runs` 원장 +
관리자 조회 정책(예방 아닌 **사후 적발**).
근본 해결은 전투 판정을 서버 권위로 옮기는 것뿐 — Edge Function 도입이나 엔진
...
3. `game_content`에 `raidTable` 시드 + 실제 레이드 보스/기믹 데이터.
   기믹 판정을 전투 결과에서 실제로 추출하는 로직(엔진 이벤트 → 기믹 id)은
   아직 설계 안 됨.
4. `web/auction.html` / `web/raid.html` UI + `nav.js` 링크.
5. **미해결 정책**: 만료된(실패한) 레이드의 기여도 처리 — 지금은 보상 없음 +
   소환템도 반환 없음(실패가 실제 손실이어야 긴장이 생긴다는 판단). 위로 보상으로
   바꾸려면 `expire_raid_instances`에서 `_settle_raid_rewards`를 축소 호출.
   **사용자 확인 필요.**
6. `_grant_item_snapshot`의 스택 병합 규칙이 `shop.html confirmPurchase()`의
   `poolRows` 필터와 **정확히 같아야 함**(name+category 일치 & held_by null &
   enhance_level null & craft_material null). 이 규칙이 한쪽에만 적용돼서
   "개조된 모자 101개" 버그가 실제로 났었다(2026-08-22). 지금은 SQL/JS 두 벌이
   존재하는 상태 — 장기적으로 SQL을 정본으로 삼고 JS를 이관하는 게 안전.
...
### 케이스 A — 30레벨 둠로드(마법공격력86, INT 75+10+6, SPD100): 4턴 처치, 문제없음
...
**⚠ SPD 계산 정정(중요 방법론 수정)**: 최초 계산 시 `effectiveSpeed`를
`effectiveSpd`와 동일하게 취급하는 실수를 했음 — 실제로는
`character.js:518-519`에 SPD 전용 감쇠 공식이 따로 있음:
`rawSpeed = baseSpeed(50) + effectiveSpd×2`, `effectiveSpeed =
10×√rawSpeed`(`dampDamageStat`의 0.6제곱과는 다른 독자적인 0.5제곱
감쇠). SPD100(bonus0)이면 `effectiveSpeed=10×√250≈158.11`(기존에
잘못 썼던 100이 아님).
...
로 **정정된** effectiveSpeed≈158.11 기준 정밀 틱 계산: t=632 SpellFocus,
t=1290 Lightning Ball①(14,760 데미지), t=1927 Mana Flood(bonusInt
6→256, effInt 91→341), t=2600 High Mana Recharge(SP회복), t=3257
Lightning Ball②(32,613 데미지, HP 52,000→4,627), t=3920 Lightning
Ball③ 첫 히트에서 사망. 엔진의 실제 턴(1,000틱 단위 전역 버킷) 기준
**4턴째** 사망(기존에 기록했던 "7턴"은 SPD 계산 오류로 인한 오답).

**결론(사용자 재확인)**: 이건 문제없다 — 레벨 상한+2차 전직+자원관리
로테이션 실행까지 다 갖춰야 나오는 "의도된 강함"이라 정당한 페이오프.
(참고: 동굴 보스는 이번 정정과 무관하게 어차피 상향 예정이라 이 턴수
자체의 정밀도는 중요하지 않음 — 정정한 이유는 SPD 공식 오류 자체를
남겨두면 다른 계산에도 반복될 위험이 있어서.)

### 케이스 A 추가 검증 — 같은 둠로드로 `"???"`(기존 최종보스) 솔로 처치 시간

사용자 요청으로 같은 둠로드 패턴으로 기존 확정 보스 `"???"`
(`unknown_entity`, HP600,000/realMdef200/SP2000/focusMana상한1000)을
공략하면 얼마나 걸리는지도 검증함(둠로드는 죽지 않는다는 가정).

**핵심 메커니즘**: `"???"`의 realMdef=200은 방어력 1단계를 완전
포화(0% 통과)시켜서, **버프 여부와 무관하게 모든 마법 히트가 정확히
raw의 10%(최소피해 규칙)만 들어감** — Mana Guard의 "받는 피해
35%감소"조차 이미 0인 값에 곱해지므로 무의미. `"???"`는
`goblin_cart`처럼 "직접 죽일 수 없는 개체"라 HP 30% 이하에서
REWARD_GRANT→RETREAT로 클리어되고, 그 전 HP 50% 이하에서
PANIC_FULL_RECOVERY(HP/SP 완전회복, 1회성)가 끼어들어 총 필요
데미지는 `300,000(100%→50%, 회복 1회) + 420,000(재차 100%→30%) =
720,000`.

**SPD 감쇠 공식(위에서 정정한 것)을 반영해 두 시나리오 비교**:
- **비상한(Mana Flood 1회만 활용, 원래 세팅)**: effectiveSpd=100 →
  effectiveSpeed≈158.11(케이스 A와 동일 캐릭터). 필요 행동 227회
  (초반 4행동 + 정상상태 14행동 사이클당 45,144데미지 반복), 사이클당
  실제 틱(LB662.5/HMR647.5)까지 반영하면 총 경과 약 149,800틱 →
  **약 150턴**.
- **전 스탯(ATK/MATK/모든 스탯) 상한(bonus=real×4) 유지**: 모든 히트가
  처음부터 최대 위력(캐스트당 28,632데미지, 10% 관통 반영) — 필요
  행동 42회. **SPD도 상한(effectiveSpd=500)으로 두면
  effectiveSpeed=10×√(50+1000)≈324.04**(⚠ 처음엔 "속도 5배=효과
  5배"로 잘못 가정해서 500을 그대로 썼었음 — 실제로는 제곱근이 걸려서
  158.11→324.04, **약 2.05배**밖에 안 빨라짐, SPD 상한버프의 체감
  효율이 생각보다 훨씬 낮다는 것도 이번에 확인됨). 사이클당 실제 틱
  (LB338.6/HMR323.6)까지 반영한 총 경과 약 14,300틱 → **약 15턴**.

**결론**: 비상한 약 150턴 vs 전 스탯 상한 약 15턴 — **약 10배 차이**
(SPD 공식을 바로잡기 전 잘못 추정했던 20배 이상보다는 줄었지만 여전히
상당한 격차). `"???"`는 이 정도의 극단적 상한버프 상황까지 감안해서
HP/MDEF가 설계돼 있다는 걸 구체적 수치로 재확인함 — 동굴 보스도
"무버프 기준"이 아니라 이런 "실전 상한버프 상황"을 기준선으로 재산정
해야 한다는 앞선 결론을 뒷받침하는 근거 자료로 남김.

### 참고 — 케이스 B(스나이퍼)의 `"???"` 솔로 처치 시간(물리 vs 마법 비대칭)

같은 스나이퍼 세팅으로 `"???"`도 확인함. 결정적 차이: Hurricane
Shot은 물리라 `takeDamage()`가 `realMdef(200, 완전 포화)`가 아니라
`realDef(15%, 정상 값)`를 씀 — 물리는 마법과 달리 `"???"`에게
포화되지 않는다. 다만 `"???"`가 개전 시(1턴) 자가 시전하는 Mana
Guard가 `physicalDamageTakenPct:35`(물리도 포함)를 주는데, `"???"`의
effectiveSpeed(≈212, SPD200 기준)가 스나이퍼(≈129.6)보다 빨라서
스나이퍼의 첫 사격 전에 이미 이 버프가 걸려있음.

히트당 데미지(DEF15%×0.85 + Mana Guard×0.65 반영): 1타 1,070 / 2~16타
각 641(기존 1회감쇠 방식) — 볼리(16히트)당 총 **10,685**.

- **SP 제한 있음(실제 상황)**: 딱 1볼리만 나가고 영구 봉쇄 —
  720,000 중 1.5%도 못 채움. **사실상 클리어 불가능.**
- **SP 무제한 가정**: 볼리당 데미지가 등속(자가 버프로 성장하는 수단이
  없어서)이라, 300,000(50%문턱)까지 29볼리 + 420,000(재차 30%문턱)까지
  40볼리 = 총 69볼리. 사이클(preDelay30+postDelay50+
  100,000/effectiveSpeed129.6≈851.5틱/볼리) 기준 총 경과 약
  58,700틱 → **약 59턴**.

**흥미로운 함의**: 물리(스나이퍼, SP 무제한 가정 시 약 59턴)가 마법
(둠로드, 비상한 약 150턴)보다 오히려 `"???"`를 더 빠르게 밀어붙인다 —
MDEF200(마법에게 포화)과 DEF15%(물리에겐 정상)의 설계 의도 차이가
수치로 명확히 드러남. 다만 실제로는 스나이퍼가 SP 관리 수단이 없어서
이 잠재력을 전혀 못 씀 — "센 스킬도 자원 관리 없인 무용"이라는 원칙이
여기서도 재확인됨. (이 항목은 기록만 — 추가 논의·조치 없음.)
...
### 조정 범위는 보스 하나로 한정 — 저층(A~I)은 그대로 둔다(사용자 확정)

"하위 단계도 더 강하게 하자면 할 수 있지만, 낮은 티어에서 그렇게까지
할 생각은 없다"— 저층(1~4층, A~I)을 더 어렵게 만들 필요는 없다는 게
명시적 결정. 대신 **보스전이 이 던전의 "주 파밍처"라서, 보스가
허무하게 무너지면 파밍 루프 자체가 의미를 잃는다는 게 진짜 우려**.
동시에 **"본격적인 패턴파훼가 다가올 2.서브티어부터 갑자기 벽을
느끼게 하고 싶지 않다"**는 난이도 곡선 설계 원칙도 재확인됨 — 저층을
지금보다 어렵게 만들면 그 완만한 램프업이 깨지므로, **이번 조정은
의도적으로 보스에만 국한**한다. (다만 "허들은 있는 편이 좋다"는
단서도 남김 — 저층에 약간의 긴장감을 주는 것 자체를 완전히 배제하진
않되, 지금 범위는 아님.)
...
## [P0] 파견 전리품 정산 — "몬스터를 몇 번 잡았는지"에 상한이 없어 희귀템 희귀도가 통째로 무력화됨 (2026-08-25, **2026-08-31 해결 — 파일 맨 위 "[P0 해결]" 섹션 참고**)
...
### 게이팅 최종 확정 — "동굴 N층 지도"(층 전용 필러 몬스터 드랍)
직전 라운드에서 임의의 열쇠 아이템(무너진 통로의 흔적 등 4종)을 만들었다가
"게이팅 아이디어만 있었지 실제 아이템은 없었다"는 사용자 정정으로 전부
되돌렸는데(위 되돌림 이력 그대로 유지), 곧바로 사용자가 실제 설계를
확정함: **"각 층에서만 등장하는 몬스터"에게 "동굴 (N)층 지도" 아이템을
만들고, `cave-floor` 2~5엔 `hasItem` 조건만 남긴다.**

- "층 전용 몬스터" = 이월되지 않는 필러 슬롯(C/E/G/I — 컨셉표에서 원래
  "N층 필러"로 라벨링된 것과 정확히 일치, A/B/D/F/H는 인접 층으로
  이월되는 축이라 "그 층에서만" 등장하지 않음):
  - C(동굴박쥐, 1층 전용) → **"동굴 1층 지도"** 드랍(확률 0.15)
  - E(동굴곰, 2층 전용) → **"동굴 2층 지도"** 드랍(확률 0.15)
  - G(동굴트롤, 3층 전용) → **"동굴 3층 지도"** 드랍(확률 0.15)
  - I(수정골렘, 4층 전용) → **"동굴 4층 지도"** 드랍(확률 0.15)
- `web/battle-themes.js`: `cave-floor-2`~`5` 요구조건을 **`hasItem`
  하나만**으로 단순화(`clearedBattle` 제거 — 지도 자체가 이미 이전 층
  공략의 증거라 중복 검사가 불필요하다는 사용자 판단). `cave-floor-2`는
  "동굴 1층 지도", ..., `cave-floor-5`는 "동굴 4층 지도" 필요. `cave-
  floor-5`는 몬스터/보스 데이터가 아직 없어 요구조건만 미리 걸어두고
  전투 실체는 다음 단계로 남김(이름도 "동굴 심층(미정)"으로 표시만
  해둠).
- `supabase/migrations/0025_...sql`: C/E/G/I 드랍테이블에 각자의 지도
  항목(`category:"keyItem"`) 추가.
...
- `web/battle-themes.js`: 새 테마 `caveTier1`("축축한 동굴") + 전투
  5개(`cave-floor-1`~`cave-floor-5`, 5는 요구조건만) 추가. 위 "동굴 N층
  지도" `hasItem` 게이팅 적용.
- `web/battle-encounters.js`: `BATTLE_MONSTER_POOLS`에 1~4층 몬스터
  풀 추가(매 층 3종, 이전 층 이월 축 재사용 — 컨셉 표 그대로). 5층은
  몬스터 풀 없음(다음 단계).
...
**배경(사용자 요청)**: "나중을 위해 상점에 아이템 판매 기능을 넣어두고
싶다. 판매금액을 세세히 정해둘 생각은 없고, 온전히 희귀도에 따라
판매가를 정할 생각이다." — 지금 당장 완성도 높은 경제 밸런스를
만드는 게 아니라, **기능 자체를 미리 심어두고 가격은 등급 하나로만
거칠게 매기는** 것이 목표.

### 설계 — `web/shop.html`에 직접 구현(실제 런타임 코드, 마이그레이션 아님)
`web/shop.html`은 상점 카탈로그(`SHOP_TABLE`)만 DB(`game_content.
shopTable`)에서 읽어오고, 구매/판매 로직 자체는 이 파일의 JS가 직접
`warehouse_items`/`profiles.gold`를 갱신하는 살아있는 코드다(구매
기능이 이미 그렇게 동작 중). 그래서 "판매"도 여기에 직접 추가함:

1. **정가 라인업 고정(사용자 후속 지시)**: "100, 500, 1000, 2000,
   5000, 10000, 100000, 1000000 중 하나로 결정되도록 미리 라인업을
   만들어두고 싶다. 그 외의 판매가격은 책정되지 않도록." — 판매가는
   반드시 이 8개 값 중 하나여야 함. 희귀도 등급 5단계(`common`(일반)/
   `uncommon`(고급)/`rare`(희귀)/`epic`(영웅)/`legendary`(전설))를
   각각 100/500/2,000/5,000/10,000G에 매핑(라인업 안의 값만 사용,
   1,000은 이번엔 안 씀). 처음엔 10/40/150/500/1500이라는 임의
   초기값으로 시작했는데, 이 지시로 전부 라인업 안의 값으로 교체함.
2. **이름 고정가 예외(`NAMED_SELL_PRICE_OVERRIDES`, 사용자 지시)**:
   희귀도 추론을 아예 건너뛰고 이름만으로 가격을 못박는 두 항목 —
   "불길한 마력 파편"(`"???"` 입장권, 원래 keyItem이라 판매 대상에서
   제외됐었는데 이 예외로 판매 가능해짐) = **100,000G**, "왕의
   대검"(고블린 왕 고유 드랍) = **1,000,000G**("보스 고유드랍 아이템은
   일괄적으로 100만 골드"라는 지시 — 지금 구현된 보스 고유 드랍은
   이것 하나뿐이라 우선 이 항목만 등록, **다음에 Heart of Deepstone
   등 새 보스 고유 아이템이 생기면 이 맵에 반드시 추가할 것**). 이
   예외가 `inferRarity`/`RARITY_SELL_PRICE`보다 항상 먼저 적용됨.
3. **`inferRarity(item)`** — 이름 예외에 안 걸린 아이템만 여기로 옴.
   아이템 자체엔 `rarity` 필드가 없는 게 대부분(기존 드랍/구매
   아이템을 전부 소급 태깅하지 않기 위한 실용적 선택)이라, 있는
   필드로 최대한 합리적으로 추론:
   - `material`: `web/material-table.js`의 `MATERIAL_TABLE[name].rarity`
     참고(이번에 "돌"/"광석"/"나무"=common, "철광석"=uncommon,
     "정동석"=rare로 태깅함). 없으면 common.
   - `equipment`: `weight`(무게)를 등급의 대리 지표로 씀 — "동굴처럼
     내구력이 오를수록 무게를 많이 먹는 트레이드오프" 설계 원칙과
     자연스럽게 맞음(0→common, 1→uncommon, 2→rare, 3→epic, 4 이상→
     legendary).
   - `keyItem`/`stash`는 판매 대상에서 제외(`null` 반환 → 목록에 아예
     안 뜸, 위 이름 예외에 등록된 것만 예외적으로 판매됨).
   - 아이템에 `rarity` 필드가 이미 있으면(나중에 필요해지면) 그걸
     최우선으로 씀 — 지금의 추론 규칙은 어디까지나 폴백.
4. **UI**: 상점 헤더에 "🛒 구매"/"💴 판매" 모드 탭 신설(기존 "⚔️ 장비"/
   "🎒 아이템" 탭과는 별개 축 — 판매는 카탈로그가 아니라 사용자의
   창고를 보여주므로). 판매 화면은 `warehouse_items`에서 장착 안 됨
   (`held_by` null)+`sellPriceFor()>0`인 것만 나열(이름 예외 덕에
   keyItem도 값이 있으면 뜸), 행마다 수량 입력 + "판매" 버튼(구매처럼
   장바구니/모달 없이 바로 확정 — "기능만 넣어두면 된다"는 이번 요청
   규모에 맞춘 단순한 구현). 이름 예외로 뜬 항목은 배지에 등급명 대신
   "고정가"라고 표시.
5. **판매 확정(`sellItem`)**: `warehouse_items.quantity`를 깎고(0이
   되면 행 삭제) `profiles.gold`에 더함 — 기존 `confirmPurchase`의
   반대 방향 패턴을 그대로 재사용.

### 검증
이 세션 환경엔 Node.js가 없어 실제 실행/`JSON.parse` 검증은 못 함 —
대신 `<script>` 블록 전체의 중괄호/소괄호 개수를 세어 균형이
맞는지(271/271, 403/403) 기계적으로 확인함. **실제 브라우저에서
구매/판매 왕복 동작(창고 갱신, 골드 반영, 열쇠 아이템이 목록에서
빠지는지, 불길한 마력 파편/왕의 대검이 고정가로 뜨는지)은 다음
세션에서 실기기 확인 필요.**

### 다음 세션에서 이어갈 것
- 실제 브라우저 테스트(관리자 계정)로 구매→판매 왕복 확인.
- **Heart of Deepstone 등 새 보스 고유 아이템이 실제로 만들어지면
  `NAMED_SELL_PRICE_OVERRIDES`에 1,000,000G로 반드시 추가할 것**(잊기
  쉬운 수동 등록 방식이라 체크리스트로 남겨둠).
- 희귀도 등급별 가격(100/500/2,000/5,000/10,000)은 초기값 — 실제
  경제 밸런스(강화 비용·파견 보상 등)와 맞춰 조정하되, **반드시 8개
  고정 라인업 안의 값만** 쓸 것.
- 장비 쪽 `weight` 기반 rarity 추론이 실제 기존 장비들(이 빠진
  도끼=weight0 등)에 적용했을 때 체감상 합리적인지 실제 창고로 확인.
