...
- 계측 전/후 로그 무변경: 스크래치 스크립트로 결정적 PRNG 고정 후 `logLines`
  파일 diff = 0 확인.
- `node index.js` + `demo-*.js`(39개) 전체 회귀 통과.
- `battle-log-render.js` 추출 후 렌더링 스모크 테스트(turn-block/outcome-title/
  skill-name-strong/action-block 마커 정상 출력 확인).
- **로그인 세션이 로컬에 없어 다음은 아직 실측 못 함**: 실제 저장→공유
  링크 생성→비로그인 열람, FIFO 10개+고정 동작, RLS 비공개 원칙(다른
  유저/anon으로 직접 테이블 조회 시 빈 배열인지) — 다음 로그인 세션에서
  반드시 확인할 것. `0021_battle_logs.sql`도 아직 실행 여부 미확인.
