...
**계층별 난이도 곡선 — 초반은 호위로 충분히 막히게, 후반부에 Enemy-All
공격 도입** (2026-08-18 확정): 동굴 초반 계층은 앞서 정한 "느리지만 맞으면
아픈 전열 히트"만으로 충분 — 호위(guardAllies) 설정이 튼튼한 파티는 큰
어려움 없이 통과하도록 함(패턴 단순 유지 원칙과 일치). **동굴 후반
계층부터 `targetFaction:"enemy", targetCount:"all"`(파티 전원에게 무조건
명중, 전열 보호 규칙 무시) 공격을 쓰는 몹을 섞기로 함** — 후열을 굳이
targetPriority로 노리는 대신, "전열 보호 자체가 안 통하는" 전체공격으로
계층이 깊어질수록 순수 화력 압박을 올리는 방향. AFTERMATH 구획은 지금까지의
관례대로 여전히 퍼즐형 전투(패턴을 읽고 대응해야 이기는 구조, "???"/고블린
수송대와 같은 설계)로 간다 — 이 원칙은 이번에 새로 정한 게 아니라 위
"난이도 곡선" 섹션에서 이미 확정돼 있던 것을 재확인함.
