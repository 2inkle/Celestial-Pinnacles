...
## 전투 로그 공유 기능 (2026-08-18 설계 논의, 계획 단계 — 아직 구현 없음)

**동기**: 동굴 4계층 "전조" 논의 중 사용자가 제안 — **전조/대비 목적과는
무관한 일반적인 기능**으로 확정(사용자 확인). "전투 하나를 통째로 서버에
저장해서, 기본은 본인만 조회 가능하되 로그의 고유 시드(id)를 아는 사람은
누구나 조회 가능하게" 하려는 것 — 예: 디스코드에 전투 결과를 링크로 공유.

**범위**: `battle-view.html`(직접 도전)만 대상 — `dispatch.html`(파견)은
`runBattle()`에 `logger: () => {}`(no-op)를 넘겨서 애초에 서사 로그 자체를
생성하지 않음(2000턴 예산 반복 시뮬 중 문자열 로그를 매번 쌓으면 성능
문제가 크기 때문, 기존 설계). 그래서 "저장 대상이 있는 전투"는 자연히
직접 도전뿐 — 파견 쪽에 별도 스코프 제외 로직을 넣을 필요가 없음.

**저장 단위**: 전투 1회 전체(사용자 확인) — `battle-view.html`이 이미
쌓고 있는 `logLines`(문자열 배열) 그대로.

**FIFO 50개 캡**: 유저당 최신 50개만 유지, 그보다 오래된 건 자동 삭제.
DB 트리거(`AFTER INSERT`)로 처리 — 매 저장 시점에 "그 유저 소유 행 중
50번째 이후"를 지움. 클라이언트가 개수를 세서 조건부로 지우는 방식(경합
가능)보다 트리거가 안전함.

**공개 범위(중요 — RLS만으로는 안전하게 구현 불가)**: "시드(고유 id)를
아는 사람은 누구나 조회 가능"을 순수 RLS(`USING (true)`)로 구현하면 **전체
테이블이 사실상 공개**된다 — PostgREST는 쿼리에 특정 id로 필터를 걸었는지
여부와 무관하게 RLS 조건만 통과하면 응답하므로, RLS가 `true`인 순간 필터
없이 전체 로그를 긁어갈 수 있음(로그인만 하면 남의 로그를 전부 열람
가능해짐 — 절대 이렇게 하면 안 됨). **표준적인 해법은 테이블 자체의
RLS는 "본인 소유만"으로 걸어 잠그고, 공유 조회는 별도의
`SECURITY DEFINER` Postgres 함수(RPC)로만 열어주는 것** — 그 함수는
정확히 하나의 id로만 단건 조회하도록 강제되므로(테이블 전체를 훑는 경로
자체가 없음), id를 모르면 조회할 방법이 원천적으로 없다. 계획:
- `battle_logs` 테이블: `user_id`, `battle_id`, `outcome`, `turns_elapsed`,
  `lines`(jsonb 배열), `created_at`. RLS는 "본인 소유만 SELECT/INSERT"
  (characters 등 기존 테이블과 동일 패턴) — 목록 화면(예: "내 전투 기록")은
  이 경로로 조회.
  - `id`(uuid, `gen_random_uuid()` 기본값) 자체를 "로그 시드"로 씀 — 별도
    토큰 컬럼 불필요(uuid 자체가 이미 충분히 추측 불가능한 값).
- `get_shared_battle_log(p_id uuid)` RPC 함수: `SECURITY DEFINER`로 그
  id 하나만 조회해서 반환(없으면 null) — RLS를 우회하지만 "정확한 id 하나"
  라는 좁은 통로만 열어주므로 안전. 공유 링크로 들어온 방문자(로그인
  여부 무관 가능)는 이 RPC만 호출.
- FIFO 트리거도 이 함수와 별개로 INSERT 시점에 `SECURITY DEFINER`로 동작.

**아직 정할 것**: 저장 트리거 방식(전투 종료 시 항상 자동 저장 vs "저장"
버튼으로 사용자가 선택), 공유 링크를 실제로 여는 뷰어 페이지(예:
`battle-log-view.html`을 살려서 쓸지 새로 만들지 — 그 페이지는 지금
`FORCED_LOG_LINES` 하드코딩 픽스처만 그리는 프리뷰용이라 실제 데이터
연결이 필요함, 위 "battle-log-view.html의 로그 파서가 낡음" 항목 참고),
익명(비로그인) 방문자도 공유 링크를 열람할 수 있게 할지. 사용자 확인 후
구현 착수.
...
**전조(대비할 시간) — 엔진 신규 개발 불필요, 기존 선딜레이 시스템 재사용**:
4계층 이후 Enemy-All 공격은 반드시 `preDelayType:"casting"`(영창형 선딜레이)을
줘서, 발동 전에 "무언가를 준비 중이다" 같은 로그가 먼저 뜨고 그 사이 최소
한 턴은 대비(회복/방어 버프/방해 스킬로 캐스팅 방해)할 여유가 생기게 함 —
"???"의 OMEN, 둠로드의 Lightning Vortex와 같은 패턴. 새 엔진 기능이 필요
없고 몬스터 스킬 설계(costs/선딜레이 수치)만으로 해결됨.
