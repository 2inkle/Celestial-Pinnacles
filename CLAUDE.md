...
1. **희귀도 등급 5단계**: `common`(일반)/`uncommon`(고급)/`rare`(희귀)/
   `epic`(영웅)/`legendary`(전설), 각각 10/40/150/500/1500G — 전부
   러프한 초기값, "세세히 정할 생각 없다"는 사용자 방침대로 나중에
   등급별 가격만 조정하면 판매 경제 전체가 같이 움직임.
2. **`inferRarity(item)`** — 아이템 자체엔 `rarity` 필드가 없는 게
   대부분(기존 드랍/구매 아이템을 전부 소급 태깅하지 않기 위한
   실용적 선택)이라, 있는 필드로 최대한 합리적으로 추론:
   ...
   - `keyItem`/`stash`는 판매 대상에서 제외(`null` 반환 → 목록에 아예
     안 뜸) — 진행에 필요한 열쇠 아이템을 실수로 팔아버리는 사고 방지.
   - 아이템에 `rarity` 필드가 이미 있으면(나중에 필요해지면) 그걸
     최우선으로 씀 — 지금의 추론 규칙은 어디까지나 폴백.
3. **UI**: 상점 헤더에 "🛒 구매"/"💴 판매" 모드 탭 신설(기존 "⚔️ 장비"/
   "🎒 아이템" 탭과는 별개 축 — 판매는 카탈로그가 아니라 사용자의
   창고를 보여주므로). 판매 화면은 `warehouse_items`에서 장착 안 됨
   (`held_by` null)+판매 가능 카테고리인 것만 나열, 행마다 수량 입력
   + "판매" 버튼(구매처럼 장바구니/모달 없이 바로 확정 — "기능만
   넣어두면 된다"는 이번 요청 규모에 맞춘 단순한 구현).
4. **판매 확정(`sellItem`)**: `warehouse_items.quantity`를 깎고(0이
...
맞는지(269/269, 390/390) 기계적으로 확인함. **실제 브라우저에서
구매/판매 왕복 동작(창고 갱신, 골드 반영, 열쇠 아이템이 목록에서
빠지는지)은 다음 세션에서 실기기 확인 필요.**

### 다음 세션에서 이어갈 것
- 실제 브라우저 테스트(관리자 계정)로 구매→판매 왕복 확인.
- 희귀도 등급별 가격(10/40/150/500/1500)은 초기값 — 실제 경제
  밸런스(강화 비용·파견 보상 등)와 맞춰 조정 필요.
- 장비 쪽 `weight` 기반 rarity 추론이 실제 기존 장비들(이 빠진
  도끼=weight0, 왕의 대검=weight2 등)에 적용했을 때 체감상 합리적인지
  실제 창고로 확인.
