...
// target(문자열 아님, 유닛 객체) — creatureTier가 "boss"면 "절대 HP/SP
// 수치"(전/후 값)만 "???"로 가리고, 이번에 가한/받은 증감량(amount)은
// 그대로 보여줌(2026-08-21, 사용자 요청: "데미지 자체는 보여줬으면
// 좋겠다. 증감치만 보이지 않으면 된다" — 총 HP/SP를 절대 수치로 역산해
// 패턴 발동 기준선을 알아내는 건 막되, 매 히트의 데미지 정보는 정상
// 노출). "더 이상 증가/감소할 수 없다"(describeStatCap) 같은 상한 알림은
// 이 함수를 안 거치는 별도 문구라 영향 안 받음 — 그건 그대로 노출.
// ⚠ 2026-08-21 1차 수정 때는 amount까지 "???"로 가렸었는데, 그러면
// "가한 데미지도 안 보인다"는 재신고로 이어짐 — before/after만 가리는
// 것으로 정정.
// ⚠ (2026-08-21 이전) 한때 빈 문자열("")을 반환해서 "가릴 수치만 빼고
// 나머지는 보여준다"가 아니라 "그 히트/행동의 로그 줄 전체가 통째로
// 사라지는" 결과가 됐던 적이 있음 — 호출부(applyDamageAndEffects 등)가
// 전부 `if (line)`으로 "내용이 있어야 로그를 남긴다"는 패턴이라, 빈
// 문자열은 "이 행동이 아예 로그에 안 남아도 된다"는 신호로 해석됐었다.
