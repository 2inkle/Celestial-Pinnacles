...
- **작업 브랜치**: `cave-lowfloor-data-authoring-2026-08-31`(9개 커밋:
  ...
  N층 지도"), `2bae660` 상점 판매 기능 신설, `a163122` companion 갱신,
  `2e27941` 판매가 라인업 고정+이름 고정가 예외 — 아래 각 항목 참고)
...
## 판매가 라인업 고정 + 이름 고정가 예외 (9번째 커밋, `2e27941`)

바로 사용자가 후속 지시: **"판매기능은 100, 500, 1000, 2000, 5000,
10000, 100000, 1000000 중 하나로 결정되도록 미리 라인업을 만들어두고
싶다. 그 외의 판매가격은 책정되지 않도록. 그리고 불길한 마력 파편
아이템을 10만에, 보스 고유드랍 아이템은 일괄적으로 100만 골드에
판매되도록 해달라."** — 7번째 커밋의 임의 초기값(10/40/150/500/1500)을
이 8개 고정 라인업 안의 값으로 전부 교체:
- `RARITY_SELL_PRICE`: common=100/uncommon=500/rare=2,000/epic=5,000/
  legendary=10,000(라인업 안의 값만, 1,000은 이번엔 안 씀).
- `NAMED_SELL_PRICE_OVERRIDES` 신설(희귀도 추론보다 우선 적용) —
  "불길한 마력 파편"(`"???"` 입장권, 원래 keyItem이라 판매 제외
  대상이었는데 이 예외로 판매 가능해짐) = 100,000G, "왕의 대검"(현재
  구현된 유일한 보스 고유 드랍) = 1,000,000G. **다음에 Heart of
  Deepstone 등 새 보스 고유 아이템이 생기면 이 맵에 반드시 추가**
  해야 함 — CLAUDE.md에 체크리스트로 남김.
- `sellableWarehouseItems()`를 `inferRarity()!==null` 대신
  `sellPriceFor()>0` 기준으로 변경 — 이름 예외로 걸린 keyItem도
  목록에 뜨도록.
- ⚠ Node.js 없어 브레이스/괄호 개수 대조(271/271, 403/403)로만 확인.
...
5. 상점 "판매" 기능 실제 브라우저 왕복 테스트(불길한 마력 파편/왕의
   대검 고정가 포함), 희귀도별 가격(100/500/2,000/5,000/10,000 —
   반드시 8개 고정 라인업 안에서만) 실제 경제 밸런스에 맞춰 조정.
6. **Heart of Deepstone 등 새 보스 고유 아이템이 생기면
   `NAMED_SELL_PRICE_OVERRIDES`(`web/shop.html`)에 1,000,000G로
   반드시 추가할 것.**
